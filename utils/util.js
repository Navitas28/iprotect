...
/**
 * Set the web3 default account to use as  public wallet address
 */
web3.eth.defaultAccount = process.env.WALLET_ADDRESS;

/**
 * The amount of ETH to be sent in this transaction
 * @type {Number}
 */
...
/**
 * Process that will run when program is  executed.
 */
const transfer = async () => {
	try {
		 ...
		 */
		let result = await web3.eth.getBalance(process.env.WALLET_ADDRESS);
		let balance = await web3.utils.fromWei(web3.utils.toBN(result), 'ether');

		log(`Destination wallet balance is currently ${balance} ETH`.green);
		...
		let details = {
			...
			nonce: nonce,
			chainId: testmode ? 3 : 1, // EIP 155 chainId - mainnet: 1, ropsten: 3
		};
		 ...
		 */
		const url = `https://rikeby.etherscan.io/tx/${transactionId}`;
		log(url.cyan);
